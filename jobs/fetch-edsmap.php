...
if(preg_match('/^[0-9A-Z]{4}$/i', $code))
{
    ...
    if($_SESSION['bgjob']['eds_mtz'])
    {
        ...
        if(!file_exists($mapName))
        {
            ...
            else $s .= "<p><div class='alert'>The mtz for $code could not be retrieved from the EDS.</div></p>\n";
        }
        else $s .= "<p><div class='alert'>The mtz for $code could not be retrieved, because a file of the same name already exists.</div></p>\n";
    }
