...
OUTPUTS / RESULTS:
    Runs Reduce on all files using the -build switch
...
foreach(listDir($inDir) as $inFile) if(endsWith($inFile, ".pdb"))
{
    ...
    if($optVerbose)
    {
      if($optNuclear) echo "reduce -build -nuclear $inFile ...\n";
      else echo "reduce -build $inFile ...\n";
    }
    if($optNuclear) $blength = 'nuclear';
    reduceBuild($inDir.'/'.$inFile, $outDir.'/'.$inFile, $blength);
}
