...
OUTPUTS / RESULTS:
    Runs Reduce on all files using "no build" settings
...
foreach(listDir($inDir) as $inFile) if(endsWith($inFile, ".pdb"))
{
    ...
    if($optVerbose)
    {
      if($optNuclear) echo "reduce -nobuild -nuclear $inFile ...\n";
      else echo "reduce -nobuild $inFile ...\n";
    }
    if($optNuclear) $blength = 'nuclear';
    reduceNoBuild($inDir.'/'.$inFile, $outDir.'/'.$inFile, $blength);
}
