    ...
    if(is_array($rota))
    {
        foreach($rota as $item)
        {
            ...
            if($item['scorePct'] <= 1.0)
            {
                ...
                if ($item['resType'] == "LEU")
                {
                   if ( ($item['chi1'] > 230  &&  $item['chi1'] < 300)  &&  
		        ($item['chi2'] > 300  ||  $item['chi2'] < 60) )
		   {
                      $reasons[] = "Likely decoy try mt";
		   }
                   if ( ($item['chi1'] > 120  &&  $item['chi1'] < 180)  &&  
			($item['chi2'] > 300  ||  $item['chi2'] < 60) )
		   {
                      $reasons[] = "Likely decoy try tt";
		   }
                   if ( ($item['chi1'] > 180  &&  $item['chi1'] < 250)  &&  
			($item['chi2'] > 150  &&  $item['chi2'] < 270) )
		   {
                      $reasons[] = "Likely decoy try tp";
		   }
                   if ( ($item['chi1'] > 260  &&  $item['chi1'] < 360)  &&  
			($item['chi2'] > 180  &&  $item['chi2'] < 300) )
		   {
                      $reasons[] = "Likely decoy try mp";
		   }
                }
                elseif ($item['resType'] == "VAL" || $item['resType'] == "THR")
                {
                   if ($item['chi1'] > 350  ||  $item['chi1'] <  10)   $reasons[] = "Likely decoy try t";
                   if ($item['chi1'] > 110  &&  $item['chi1'] < 130)   $reasons[] = "Likely decoy try m";
                   if ($item['chi1'] > 230  &&  $item['chi1'] < 250)   $reasons[] = "Likely decoy try p";
                }
                else
                {
		...
		if ($reasons) $res[$item['resName']]['rota_isdecoy'] = true;
		else $res[$item['resName']]['rota_isbad'] = true;

                $res[$item['resName']]['rota'] = "$item[scorePct]%<br><small>chi angles: ".formatChiAngles($item)."<br>".implode(", ", $reasons)."</small>";
                $res[$item['resName']]['any_isbad'] = true;
            }
            else
...
//    if(is_array($rota))   $header1[] = array('html' => "<b>Decoy Rotamer</b>",          'sort' => 1); 
...
//      if(is_array($rota))   $header2[] = array('html' => "Outliers: ".count(findRotaDecoy($rota))." of ".count($rota));
      ...
      foreach($res as $cnit => $eval)
      {
        ...
        foreach(array('clash', 'rama', 'rota', 'cbdev', 'pperp', 'suites', 'bbonds', 'bangles') as $type)
        {
          if(is_array($$type))
          {
            ...
            if(isset($eval[$type.'_isdecoy']))  $cell['color'] = '#ffff99';
