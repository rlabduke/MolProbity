...
function runAnalysis($modelID, $opts)
{
    ...
    if($opts['chartImprove'] && ($clash || $rota))
    {
        ...
        if($model['isFixed'] && $model['isBuilt'])
        {
...
echo "Both ".$altInpath; 
            ...
            // Rotamers Decoys
                $outfileD = mpTempfile("tmp_rotamerD_");
                runRotamer($altpdb, $outfile);
                $altrota = loadRotamer($outfile);
                $altRotaDecoyCount = count(findRotaDecoys($altrota));
...
echo "Built ".$altInpath; 
            ...
            if(count($improvementList) > 0)
            {
                ...
                $improveText .= implode(" and ", $improvementList);
                $improveText .= ".  <b>Make sure you download the modified PDB to take advantage of these improvements!</b></div>\n";
                if (count($suggestionsList) > 0) 
                { 
                   $improveText .= "<div class='feature'>"; 
                   $improveText .= implode(" ", $suggestionsList);
                   $improveText .= "</div>\n";
                }
            }
            elseif(count($suggestionsList) > 0)
            {
               $improveText .= "<div class='feature'>";  
               $improveText .= implode(" ", $suggestionsList);
               $improveText .= "</div>\n";
...
#{{{ findRotaDecoys - evaluates residues for decoy rotamers
############################################################################
/**
* Returns an array of 9-char residue names for residues that
* fall outside the allowed boundaries for this criteria.
...
function findRotaDecoys($rota)
{
    ...
    if(is_array($rota)) foreach($rota as $res)
    {
       if($res['scorePct'] <= 1.0)
       {
           if ($res['resType'] == "LEU")
           {
              if ( ($res['chi1'] > 230  &&  $res['chi1'] < 300)  &&
                   ($res['chi2'] > 300  ||  $res['chi2'] < 60) )
              {
                 $worst[$res['resName']] = "Likely decoy try mt";
              }
              if ( ($res['chi1'] > 120  &&  $res['chi1'] < 180)  &&
                   ($res['chi2'] > 300  ||  $res['chi2'] < 60) )
              {
                 $worst[$res['resName']] = "Likely decoy try tt";
              }
              if ( ($res['chi1'] > 180  &&  $res['chi1'] < 250)  &&
                   ($res['chi2'] > 150  &&  $res['chi2'] < 270) )
              {
                 $worst[$res['resName']] = "Likely decoy try tp";
              }
              if ( ($res['chi1'] > 260  &&  $res['chi1'] < 360)  &&
                   ($res['chi2'] > 180  &&  $res['chi2'] < 300) )
              {
                 $worst[$res['resName']] = "Likely decoy try mp";
              }
           }
           elseif ($res['resType'] == "VAL" || $res['resType'] == "THR")
           {
              if ($res['chi1'] > 330  ||  $res['chi1'] <  30)   $worst[$res['resName']] = "Likely decoy try t";
              if ($res['chi1'] >  90  &&  $res['chi1'] < 150)   $worst[$res['resName']] = "Likely decoy try m";
              if ($res['chi1'] > 210  &&  $res['chi1'] < 270)   $worst[$res['resName']] = "Likely decoy try p";
           }
       }
    }
    ksort($worst); // Put the residues into a sensible order
